...
class SelectManyWithFormGenerator {
  ...
  generate() {
    ...
    _.forEach(model.fields, (field, fieldName) => {
      ...
      if(field.type === 'relationship' && field.relationshipType === 'oneToMany') {
        ...
        const { Component: Select } = amon.getComponent(componentsNameGenerator.SelectManyModel(model, field, fieldName));
        ...
        class GeneratedComponent extends Component {
          ...
          handleSubmit(formSituation, formProps, newRecord) {
            ...
            const currentValues = parentForm.getFieldValue(finalFieldName) || [];
            parentForm.setFieldsValue({ [finalFieldName]: [...currentValues, newRecord.id] });
          }
        ...
        amon._addComponent(componentsNameGenerator.SelectManyModelWithForm(fieldSituation), GeneratedComponent);
      }
    });
...
export default SelectManyWithFormGenerator;
